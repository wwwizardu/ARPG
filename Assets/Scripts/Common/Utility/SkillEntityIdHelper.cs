...
namespace ARPG.Utility
{
    ...
    public static class SkillEntityIdHelper
    {
        ...
        public static bool GetSkillCommandComponent(int slotIndex, int entityId, Vector2 targetPosition, out SkillCommandComponent command)
        {
            ...
            int skillEntityId = GetSkillEntityId(entityId, slotIndex);
            if (skillEntityId == -1)
            {
                return false;
            }
            ...
            {
                Debug.LogWarning($"[SkillEntityIdHelper] Skill not found at slot {slotIndex}");
                return false;
            }
            ...
            {
                Debug.LogError($"[SkillEntityIdHelper] Skill.Table is null for SkillId({skill.SkillId}), SkillEntityId({skillEntityId})");
            }
